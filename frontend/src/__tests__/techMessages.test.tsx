...
describe("Технические сообщения — единственное место вывода <Notice />", () => {
	...
	it("замолчавший источник забирает свою запись с собой", () => {
		...
		/*
		 * Записи больше нет, и это не потеря.
		 *
		 * Сообщение формы — СОСТОЯНИЕ, а не событие: «агент не на связи» значит «сейчас не
		 * на связи». Источник замолчал — значит, это перестало быть правдой, и оставлять
		 * строку историей означало бы показывать неверное наравне с верным. То, что
		 * случилось и должно остаться (отказ команды, потеря связи), заводят `addMessage` и
		 * `noteNotice`, и их история никуда не девается.
		 */
		expect(screen.queryByText("Базы: Агент не на связи")).toBeNull();
	});
});
...
describe("Технические сообщения: сводка формы", () => {
	...
	it("меняющийся счётчик правит свою запись, а не заводит новую", () => {
		/*
		 * ЖИВОЙ СЛУЧАЙ 12.09: снимая отметки прав, человек получал ленту из одиннадцати
		 * «Не применено правок: N» — по записи на каждое нажатие, и десять из них заведомо
		 * неверные. Сообщение формы — состояние: изменилось число, значит изменилась ТА ЖЕ
		 * запись, а не появилась новая.
		 */
		const say = (n: number) => act(() => {
			reportNotices("pane-1", "k", "Карточка", [{ type: "info", text: `Не применено правок: ${n}` }]);
		});
		say(6);
		...
		const only = getMessages()[0];
		expect(only.text).toBe("Не применено правок: 1");
		// Та же самая запись: и по личности, и по времени появления — «висит с обеда».
		expect(only.id).toBe(first.id);
		expect(only.firstAt).toBe(first.firstAt);
	});

	it("правка текста не переставляет запись в списке", () => {
		act(() => {
			reportNotices("pane-1", "k", "Карточка", [
				{ type: "error", text: "Не заполнено имя" },
				{ type: "info", text: "Не применено правок: 2" },
			]);
		});
		act(() => {
			reportNotices("pane-1", "k", "Карточка", [
				{ type: "error", text: "Не заполнено имя" },
				{ type: "info", text: "Не применено правок: 3" },
			]);
		});
		// Порядок прежний: обновлённая строка не прыгает наверх, как новая.
		expect(getMessages().map((m) => m.text)).toEqual([
			"Не заполнено имя", "Не применено правок: 3",
		]);
	});

	it("исчезнувшее из сводки не остаётся следом, остальное остаётся актуальным", () => {
		act(() => {
			reportNotices("pane-1", "k", "Карточка", [
				{ type: "info", text: "Правка заблокирована" },
				{ type: "info", text: "Не применено правок: 1" },
		...
		expect(byText.has("Правка заблокирована")).toBe(false);
