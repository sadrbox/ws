...
const FORM_LABEL = "Счёт на оплату";
...
const PaymentInvoicesForm: FC<Partial<TPane>> = ({ onSave, onClose, data, uniqId }) => {
  ...
  const [formData, setFormData, clearFormStorage, hadStoredData] = useFormSessionStore<TFormData>(
    "payment-invoices-form", uuid ?? "new", initialForm,
  );
...
interface PaymentInvoicesListProps { variant?: TTableVariant; onSelectItem?: (item: TDataItem) => void; ownerUuid?: string; ownerField?: string; }

const PaymentInvoicesList: FC<PaymentInvoicesListProps> = ({ variant = "default", onSelectItem, ownerUuid, ownerField } = {}) => {
