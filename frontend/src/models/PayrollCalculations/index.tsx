...
const PayrollCalculationsForm: FC<Partial<TPane>> = (paneProps) => {
  ...
  const notices = useDocumentNotices({ docType: "payroll_calculation", fields: form.fields as unknown as Record<string, unknown>, formError: form.errorKind === "form" ? form.error : null });
        ...
        <div className={styles.FormContainer}>
          <div className={styles.FormWrapper}>
            <GroupCol className={styles.Form}>
              ...
              <Group>
                <FormLookup form={form} field="position" endpoint="positions" label="position.name" />
              </Group>
              <Divider />
  ...
  ], [form.fields, form.formUid, form.isLoading, form.isEditMode, form.setField, form.setFields, handleSalaryChange, assignNumber, canWrite, notices]);
  ...
  const headerActionsPortal = usePaneHeaderActions(
    ...
    (
      <>
        ...
        {isSavedDoc && <DocumentEntriesButton documentType="payroll_calculation" documentUuid={form.fields.uuid} />}
      </>
    ),
  );
...
const PayrollCalculationsList: FC<{ variant?: TTableVariant; onSelectItem?: (item: TDataItem) => void; ownerUuid?: string; ownerField?: string; extraQueryParams?: Record<string, string> }> = ({ variant, onSelectItem, ownerUuid, ownerField, extraQueryParams }) => (
