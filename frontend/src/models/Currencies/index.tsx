...
const EMPTY_FORM: TFormData = { code: "", shortName: "", symbol: "" };

const CurrenciesForm: FC<Partial<TPane>> = ({ onSave, onClose, data, uniqId }) => {
  const uuid = data?.uuid as string | undefined;
  const { windows: { removePane, updatePaneLabel } } = useAppContext();
  const formUid = useUID();

  const [formData, setFormData, clearFormStorage, hadStoredData] = useFormSessionStore<TFormData>(
    "currencies-form", uuid ?? "new", EMPTY_FORM,
  );
  ...
  const loadFormData = useCallback(async (entityUuid: string) => {
    ...
    try {
      ...
      setFormData({ code: d.code ?? "", shortName: d.shortName ?? "", symbol: d.symbol ?? "", id: d.id, uuid: d.uuid });
  ...
  const submit = useCallback(async (): Promise<boolean> => {
    setIsLoading(true); setError(null);
    if (!formData.code?.trim()) { setError("Код валюты обязателен"); setIsLoading(false); return false; }
    if (!formData.shortName?.trim()) { setError("Наименование обязательно"); setIsLoading(false); return false; }
    const payload = { code: formData.code.trim(), shortName: formData.shortName.trim(), symbol: formData.symbol?.trim() || null };
    try {
      ...
      setFormData(prev => ({ ...prev, ...saved, code: saved.code ?? "", shortName: saved.shortName ?? "", symbol: saved.symbol ?? "" }));
      setIsEditMode(true);
      if (uniqId) updatePaneLabel(uniqId, `${translate(LIST_NAME) || FORM_LABEL}: ${saved.code || "?"} — ${saved.shortName || "?"} • ${saved.id ?? "?"}`);
      onSave?.(); return true;
    ...
    <div className={styles.FormBodyParts}>
              <Group align="row" gap="12px" className={styles.Form}>
                <div style={{ display: "flex", flexDirection: "column", gap: "12px", flex: 1 }}>
                  <Field label="Код валюты (ISO) *" name={`${formUid}_code`} minWidth="150px"
                    value={formData.code} onChange={e => handleFieldChange("code", e.target.value)} disabled={isLoading} />
                  ...
                  <Field label="Символ" name={`${formUid}_symbol`} minWidth="100px"
                    value={formData.symbol} onChange={e => handleFieldChange("symbol", e.target.value)} disabled={isLoading} />
                </div>
              </Group>
...
interface CurrenciesListProps { variant?: TTableVariant; onSelectItem?: (item: TDataItem) => void; }

const CurrenciesList: FC<CurrenciesListProps> = ({ variant = "default", onSelectItem } = {}) => {
  ...
  const openModelForm = useCallback((formProps: TOpenModelFormProps) => {
    ...
    const isEdit = !!d?.uuid;
    addPane({
      label: isEdit ? `${t(componentName)}: ${d?.code || "?"} — ${d?.shortName || t("noName")} • ${d?.id ?? "?"}` : `${t(componentName)}: ${t("new")}`,
      component: CurrenciesForm, data: d, onSave: () => refetch(), onClose: () => refetch(),
    });
