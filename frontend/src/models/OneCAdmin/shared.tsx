 ...
 * `publish` / `unpublish` — сама публикация и её снятие: применимы к живым базам,
 *        а текущее состояние публикации проверяется отдельно, чтобы можно было назвать
 *        причину пропуска («уже опубликована»), а не молча выбросить базу из списка.
 */
