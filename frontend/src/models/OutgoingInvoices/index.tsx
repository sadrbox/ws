...
const OutgoingInvoicesForm: FC<Partial<TPane>> = ({ onSave, onClose, data, uniqId }) => {
...
interface OutgoingInvoicesListProps { variant?: TTableVariant; onSelectItem?: (item: TDataItem) => void; ownerUuid?: string; ownerField?: string; ownerName?: string; }

const OutgoingInvoicesList: FC<OutgoingInvoicesListProps> = ({ variant = "default", onSelectItem, ownerUuid, ownerField, ownerName } = {}) => {
  ...
  const openModelForm = useCallback((formProps: TOpenModelFormProps) => {
    ...
    addPane({ label: `${t(componentName)}: ${title} • ${d?.id ?? "?"}`, component: OutgoingInvoicesForm, data: newData, onSave: () => refetch(), onClose: () => refetch() });
...
OutgoingInvoicesList.displayName = "OutgoingInvoicesList";
