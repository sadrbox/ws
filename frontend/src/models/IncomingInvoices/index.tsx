  ...
  printConfig: {
    buildLayout: (fields, items, cols) => (
      <OutgoingInvoicePrint data={{
        ...
        totalAmountWithoutVat: items.reduce((s: number, r: TDataItem) => s + Number(r.amountWithoutVat ?? 0), 0),
        totalVatAmount: items.reduce((s: number, r: TDataItem) => s + Number(r.vatAmount ?? 0), 0),
        totalDiscountAmount: items.reduce((s: number, r: TDataItem) => s + Number(r.discountAmount ?? 0), 0),
        totalExciseAmount: items.reduce((s: number, r: TDataItem) => s + Number(r.exciseAmount ?? 0), 0),
