...
const IncomingInvoicesForm: FC<Partial<TPane>> = createInvoiceLikeForm({
  ...
  accessPermissionModel: "IncomingInvoice",
  ...
  printConfig: {
    buildLayout: (fields, items, cols) => (
      <OutgoingInvoicePrint data={{
        ...
        totalAmountWithoutVat: items.reduce((s, r) => s + Number(r.amountWithoutVat ?? 0), 0),
        totalVatAmount: items.reduce((s, r) => s + Number(r.vatAmount ?? 0), 0),
        totalDiscountAmount: items.reduce((s, r) => s + Number(r.discountAmount ?? 0), 0),
        totalExciseAmount: items.reduce((s, r) => s + Number(r.exciseAmount ?? 0), 0),
...
const IncomingInvoicesList: FC<{ variant?: TTableVariant; onSelectItem?: (item: TDataItem) => void; ownerUuid?: string; ownerField?: string; extraQueryParams?: Record<string, string> }> = (
