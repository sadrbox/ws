...
const IncomingInvoicesForm: FC<Partial<TPane>> = ({ onSave, onClose, data, uniqId }) => {
...
IncomingInvoicesForm.displayName = "IncomingInvoicesForm";

const stringifyJson = (v: any): string => { if (v == null) return ""; try { const s = JSON.stringify(v); return s === "{}" || s === "[]" ? "" : s; } catch { return ""; } };
interface IncomingInvoicesListProps { variant?: TTableVariant; onSelectItem?: (item: TDataItem) => void; ownerUuid?: string; ownerField?: string; ownerName?: string; }

const IncomingInvoicesList: FC<IncomingInvoicesListProps> = ({ variant = "default", onSelectItem, ownerUuid, ownerField, ownerName } = {}) => {
  ...
  const openModelForm = useCallback((formProps: TOpenModelFormProps) => {
    ...
    addPane({ label: `${t(componentName)}: ${title} • ${d?.id ?? "?"}`, component: IncomingInvoicesForm, data: newData, onSave: () => refetch(), onClose: () => refetch() });
...
IncomingInvoicesList.displayName = "IncomingInvoicesList";
