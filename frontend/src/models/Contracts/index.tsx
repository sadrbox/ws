...
const DEFAULT_PARAMS: TypeTableParams = {
  ...
  filter: { searchBy: { columns: [], value: "" } }, // Уточняем структуру фильтра по умолчанию
...
const Contracts: FC = () => {
  ...
  const model = "Contracts"; // Лучше явно указать строку, чем полагаться на .name
  ...
  useEffect(() => {
    if (error) {
      console.error("React Query Error fetching Contracts:", error);
