...
interface TFields { id?: number; uuid?: string; shortName: string; description: string; }
const DEFAULT_FIELDS: TFields = { shortName: "", description: "" };

const PositionsForm: FC<Partial<TPane>> = (paneProps) => {
  const { canWrite } = useAccessRight("Position");
  const form = useFormStore<TFields>({
    endpoint: MODEL_ENDPOINT, storageKey: "positions-form", defaultFields: DEFAULT_FIELDS, paneProps,
    mapServerToForm: (d, prev) => ({ ...(prev ?? DEFAULT_FIELDS), ...d, shortName: d.shortName ?? "", description: d.description ?? "" }),
    buildPayload: (fd) => {
      if (!fd.shortName?.trim()) return "Наименование обязательно";
      return { shortName: fd.shortName.trim(), description: fd.description.trim() || null };
    },
    buildPaneLabel: (saved) => `${translate(LIST_NAME) || "Должности"}: ${saved.shortName || "?"} • ${saved.id ?? "?"}`,
  });
  ...
  const tabs = useMemo(() => [
    { id: "general", label: translate("general") || "Общие сведения", component: (
      <div className={styles.FormBodyParts}>
        ...
        <Group align="row" gap="12px" className={styles.Form}>
          <Field label="Описание" name={`${form.formUid}_description`} minWidth="339px" value={form.fields.description} onChange={e => form.setField("description", e.target.value)} disabled={form.isLoading} />
        </Group>
...
const PositionsList: FC<{ variant?: TTableVariant; onSelectItem?: (item: TDataItem) => void }> = ({ variant, onSelectItem }) => (
  <ModelList endpoint={MODEL_ENDPOINT} listName={LIST_NAME} columnsJson={columnsJson} FormComponent={PositionsForm}
