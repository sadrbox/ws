...
export const BaseAvailability: FC<{
	...
	/*
	 * РЕГИСТРАЦИИ В КЛАСТЕРЕ НЕТ (П32). Такой базе не нужны ни «Удалить базу из кластера 1С» (удалять там нечего —
	 * агент ответил бы «не найдена в кластере»), ни скрытие (работать с ней нельзя в любом случае). Нужно одно —
	 * убрать её запись из панели (С45).
	 *
	 * ДВЕ РАЗНЫЕ КОМАНДЫ — РАЗВЕДЕНЫ ПО ТОМУ, ГДЕ ДЕЙСТВУЮТ (17.09): «Удалить базу из кластера 1С» меняет сервер и
	 * видна всем инструментам, «Убрать базу из панели» удаляет только нашу запись. По прежним подписям («Удалить
	 * регистрацию из кластера» и «Убрать из списка») это различие приходилось угадывать.
	 */
