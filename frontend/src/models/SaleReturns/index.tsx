...
/** Сид панели формы возврата от покупателя (paneProps.data). */
interface SaleReturnPaneData {
  uuid?: string;
...
/** Серверная запись документа возврата (вход mapServerToForm). */
interface SaleReturnServerRecord {
  id?: number;
  uuid?: string;
  number?: string | null;
  date?: string | null;
...
/** Строка возврата для печати (live-строки таблицы с relation-объектами). */
interface SaleReturnItemRow extends TDataItem {
...
const SaleReturnsForm: FC<Partial<TPane>> = (paneProps) => {
  ...
  const initialFields: TFields | undefined = (() => {
    const data = paneProps.data as SaleReturnPaneData | undefined;
  ...
  const form = useFormStore<TFields>({
    ...
    tables: {
      items: {
        ...
        extraSkipFields: ["saleReturnUuid"],
      },
    },
    mapServerToForm: (d: SaleReturnServerRecord, prev) => ({
    ...
    buildPayload: (fd) => {
      const validation = validateDocumentFields("sale_return", fd as unknown as Record<string, unknown>);
  ...
  const handlePrint = useCallback(() => {
    ...
    const rows = allItemsRef.current.map((raw, i) => {
      const r = raw as SaleReturnItemRow;
      return {
      number: i + 1,
    ...
    const titleStr = `Возврат от покупателя № ${form.fields.id ?? "—"}`;
    const fileBase = `ВозвратПокуп_${form.fields.id ?? "новый"}`;
    addPane({
  ...
  const headerActionsPortal = usePaneHeaderActions(
    ...
    (isSavedDoc || hasBasis) ? (
      <>
        {isSavedDoc && <DocumentChainButton documentType="sale_return" documentUuid={form.fields.uuid} />}
        {isSavedDoc && <DocumentEntriesButton documentType="sale_return" documentUuid={form.fields.uuid} />}
