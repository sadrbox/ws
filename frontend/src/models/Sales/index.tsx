...
const SalesForm: FC<Partial<TPane>> = (paneProps) => {
  ...
  const handleGovDoc = useCallback(async (id: string) => {
    ...
    if (!uuid) return;
    // T7.8: построчные ошибки отклонения СНТ/ЭАВР. Тост — коротко, сколько ошибок; весь
    // список — в журнал формы: десяток строк за четыре секунды не прочитать.
    const showErrors = (errs?: { errorCode?: string | null; text?: string | null; property?: string | null }[]) => {
      ...
      notify({
        severity: "error", text: msg, source: translate("sale"), scope: form.paneId,
        toast: `${translate("govDeclined")}: ${errs.length}`,
      });
    };
    try {
