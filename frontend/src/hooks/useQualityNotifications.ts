 ...
 *
 * ДВА КАНАЛА, И ОБА НУЖНЫ. SSE-событие `notify` приходит мгновенно, но шина событий на сервере
 * живёт в памяти процесса, а backend в production работает кластером: событие из соседнего
 * процесса до этого клиента не доходит. Поэтому раз в минуту — опрос непрочитанных «с момента
 * последней проверки»; уже показанное не повторяется (множество показанных uuid).
 *
