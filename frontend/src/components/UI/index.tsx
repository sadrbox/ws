...
/**
 * Минимум обеих областей рабочего пространства — и пейнов, и области сообщений.
 *
 * 240 пикселей — не круглое число ради красоты: это ширина, ниже которой список перестаёт
 * быть списком (тулбар переносится в три строки, колонки схлопываются), а форма — формой.
 * То же значение стоит минимумом в вёрстке обеих областей: правило одно, записано дважды —
 * здесь им ограничивается перетаскивание, там оно страхует изменение размера окна.
 */
...
export const Container: FC = () => {
   ...
   */
  /*
   * ПРЕДЕЛ — В ПИКСЕЛЯХ, А НЕ В ДОЛЯХ. Верхней границы у области больше нет: человек вправе
   * отдать ей хоть три четверти экрана. Ограничение одно и понятное: ни область, ни пейны не
   * становятся уже 240 пикселей — в такой полосе не помещается ни строка таблицы, ни кнопка.
   * Проценты остаются только формой хранения: окно меняет ширину, и доля переживает это
   * лучше, чем запомненные пиксели.
   *
   * `cssVar` — то же имя переменной, что стоит на контейнере ниже: во время перетаскивания
   * хук пишет размер прямо в неё, минуя перерисовку React (иначе каждое движение указателя
   * перерисовывало бы всю область вместе с открытой в ней перепиской или списком задач).
   */
