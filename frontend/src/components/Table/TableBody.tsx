...
const TableBodyRow: FC<TableBodyRowProps> = memo(({ row, columns, isActive, isSelected, rowIndex, activeCellId, isAllSelectedMode, isChild, onToggleSelect }) => {
  ...
  /*
   * НЕЧЕГО ОТМЕЧАТЬ — НЕТ И ЧЕКБОКСА, но КЛЕТКА остаётся: убрать её значило бы сделать в
   * строке на одну ячейку меньше, чем у соседей, — колонки разъедутся ровно так же, как
   * разъезжались, когда групповая строка рисовалась своей разметкой.
   *
   * Два случая. Первый — ПОЯСНЯЮЩИЙ потомок: он только рассказывает о своей группе.
   * Второй — строка, которую сам список объявил неотмечаемой (`__inert`): так «Задания»
   * помечают задание, чьи команды уже не найти, — отменять нечего, повторять нечего.
   * Чекбокс, который щёлкается и ничего не делает, хуже отсутствующего: он обещает.
   */
  const checkboxIsInert = (isChild && row.__selected === undefined) || row.__inert === true;
      ...
      >
        {showCheckbox && (
          ...
          >
            ...
            >
              {/*
                * У строки, над которой действовать нечем, вместо чекбокса стоит прочерк.
                * Пустая клетка читается как «чекбокс пропал» и заставляет гадать, почему у
                * соседней строки он есть; прочерк с подсказкой отвечает на это прямо.
                */}
              {checkboxIsInert && (
                <span className={styles.InertMark} title={translate("rowNotSelectable")}>—</span>
              )}
