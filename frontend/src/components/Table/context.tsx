...
export interface TableContextProps {
  ...
  selectable: boolean;
  /**
   * Отметки ВИДНЫ, но недоступны — на время операции над теми же данными.
   *
   * Раньше на это время колонку отметок прятали целиком (`selectable: false`): таблица
   * перестраивалась, колонки съезжали влево, а по окончании прыгали обратно — и человек
   * терял, где что было отмечено. Недоступный чекбокс говорит то же самое («сейчас
   * менять нельзя»), не двигая разметку.
   */
   ...
   *
   * Пустая таблица молчит, а молчание двусмысленно: «расширений у базы нет» и «их ещё никто
   * не читал» выглядят одинаково, хотя второе требует действия человека. Раньше такие
   * объяснения уходили в «Технические сообщения» и висели там, пока открыта форма: очистка
   * их не брала, и список сообщений выглядел незакрывающимся. Место объяснения — там, где
   * человек ищет данные, то есть в самой таблице.
   */
  emptyText?: string;
  /**
   ...
   * сообщения, ответ агента, причина отказа. Обрезать их многоточием значит спрятать
   * ровно то, ради чего в таблицу и смотрят. Вместе с переносом выключается
   * виртуализация: она верит, что все строки одной высоты.
   */
  wrapCells?: boolean;
