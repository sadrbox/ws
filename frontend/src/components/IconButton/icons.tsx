...
/** Открыть элемент — стрелка «наружу вверх-вправо». Без рамки: две линии, максимум
 *  смысла. Универсальный жест «перейти / открыть», не спорит с рамкой поля. */
export const OpenIcon: FC<SvgProps> = (props) => (
  <svg {...baseProps} {...props}>
    <path d="M4.75 11.25L11.25 4.75" />
    <path d="M6.5 4.75H11.25V9.5" />
  </svg>
);
...
/** Перезаполнить по основанию — ОДНА круговая стрелка, без документа внутри.
 *  Кнопка стоит в ряду действий поля рядом с ⌄ (быстрый выбор), ↗ (открыть),
 *  ☰ (список) и ✕ (очистить) — это глифы в один-два штриха, без замкнутых форм
 *  и мелких деталей. Мини-документ с текстовыми линиями относится к другому
 *  визуальному классу (подробная пиктограмма) и выбивался из ряда: в 20px он
 *  превращался в тёмное пятно рядом с лёгкими соседями. Что именно
 *  перезаполняется, сообщает подсказка, а не рисунок.
 *  Габарит 3.7–12.3 — как у соседей (✕ занимает 3.5–12.5). */
export const SyncFromBasisIcon: FC<SvgProps> = (props) => (
  <svg {...baseProps} {...props}>
    <path d="M8 3.7A4.3 4.3 0 1 0 11.55 5.55" />
    <path d="M9.4 2.6 8 3.7l1.4 1.3" />
  </svg>
);
