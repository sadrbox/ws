...
export function getByEndpoint(endpoint: string): ModelRegistryEntry | undefined {
  return byEndpoint.get(endpoint);
}

/** Получить запись по storageKey (например "organizations-form", "cash-receipt-orders-form") */
export function getByStorageKey(storageKey: string): ModelRegistryEntry | undefined {
 ...
 */
export async function loadFormByEndpoint(endpoint: string): Promise<FC<any> | undefined> {
 ...
 * Возвращает React FC или undefined, если endpoint не найден в реестре.
 */
export async function loadListByEndpoint(endpoint: string): Promise<FC<any> | undefined> {
  const entry = byEndpoint.get(endpoint);
  if (!entry) return undefined;
  const mod = await entry.module();
