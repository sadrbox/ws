...
router.get(`/${ROUTE}`, async (req, res) => {
	try {
		...
		if (orderBy.length === 0) orderBy.push({ rate: "asc" });
		...
		const baseWhere = { deletedAt: null, ...searchWhere, ...filterWhere };
...
router.post(`/${ROUTE}`, async (req, res) => {
	try {
		...
		if (rate === undefined || rate === null || isNaN(Number(rate)))
			return res
				.status(400)
				.json({ success: false, message: "Ставка НДС обязательна" });
...
router.put(`/${ROUTE}/:id`, async (req, res) => {
	try {
		...
		if (req.body.rate !== undefined && !isNaN(Number(req.body.rate)))
