...
router.post(`/${ROUTE}`, async (req, res) => {
	try {
		...
		const docNumber = await ensureDocumentNumber({ docType: "payment_invoice", modelName: MODEL, manual: req.body.number, organizationUuid, date });
...
router.put(`/${ROUTE}/:id`, async (req, res) => {
	try {
		...
		const _ex = await prisma[MODEL].findUnique({ where: w, select: { uuid: true, organizationUuid: true, number: true, contractUuid: true, date: true } });
		...
		{
			const _num = await ensureDocumentNumber({ docType: "payment_invoice", modelName: MODEL, manual: data.number !== undefined ? data.number : _ex?.number, organizationUuid: data.organizationUuid ?? _ex?.organizationUuid, date: data.date ?? _ex?.date, excludeUuid: _ex?.uuid });
...
router.delete(`/${ROUTE}/:id`, (req, res) =>
	handleDelete({ req, res, prisma, modelName: MODEL, numberDocType: "payment_invoice" }),
);
router.post(`/${ROUTE}/batch-delete`, (req, res) =>
	handleBatchDelete({ req, res, prisma, modelName: MODEL, numberDocType: "payment_invoice" }),
);
