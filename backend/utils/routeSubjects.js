...
export const ROUTE_SUBJECTS = {
	...
	// Роль в учёте качества задают ГРУППЫ СОТРУДНИКОВ (главбух, руководитель, участники), а не
	// права на модели: services/quality/access.js решает, кто что видит и подтверждает.
	// Данные клиентов (находки, выписки КН, первичка) — дополнительно по организации.
	quality: { kind: "guarded", note: "E17: контекст, настройки, уведомления, бонусы, панели — роль по группам сотрудников" },
	"standard-violations": { kind: "guarded", note: "E17: реестр нарушений — видимость и решения по группам, о себе решает только уровень выше" },
	"violation-measures": { kind: "guarded", note: "E17: меры руководителя — назначает тот, кто решает по сотруднику" },
	"standard-items": { kind: "guarded", note: "E17: справочник пунктов стандарта фирмы — правят администратор и руководитель" },
	"staff-groups": { kind: "guarded", note: "E17: группы сотрудников фирмы — правят администратор и руководитель" },
	"error-types": { kind: "guarded", note: "E17: типовые ошибки — ведут главбух, руководитель, администратор" },
	"checklist-templates": { kind: "guarded", note: "E17: шаблоны чек-листов фирмы — ведут главбух, руководитель, администратор" },
	"checklist-runs": { kind: "guarded", note: "E17: чек-листы по клиентам — исполнитель, главбух, группа клиента" },
	attendance: { kind: "guarded", note: "E17: отметка начала дня и журнал — своё всегда, чужое по группам" },
	"work-schedules": { kind: "guarded", note: "E17: графики сотрудников — задаёт тот, кто решает по сотруднику" },
	"absence-requests": { kind: "guarded", note: "E17: заявки на опоздание и отсутствие — свои всегда, согласование по группам" },
	"work-calendar": { kind: "guarded", note: "E17: производственный календарь РК — читают все, правит администратор фирмы" },
	"check-findings": { kind: "guarded", note: "E17: находки проверок учёта 1С — по организации клиента, решения по группам" },
	"check-runs": { kind: "guarded", note: "E17: журнал прогонов проверок учёта — по организации клиента" },
	"kn-statements": { kind: "guarded", note: "E17: выписки лицевого счёта КН и сверка с 1С — по организации клиента" },
	"primary-docs-receipts": { kind: "guarded", note: "E17: отметки о получении первички — по организации клиента" },
	// Служебный канал AI-сервиса (/bpai, ключ X-Api-Key): итоги ночного прогона проверок учёта.
	checks: { kind: "operator", note: "E17: приём итогов проверок учёта от сервиса ai по ключу канала /bpai, пользователя нет" },
