...
test("Жёсткий резерв: уменьшает доступный остаток; резерв-основание реализации исключается", async (t) => {
	...
	// Резерв 30 ед. по этому товару/складу. ПРОВЕДЁН — иначе регистр не движется.
	const reservation = await prisma.reservation.create({
		data: {
			date: new Date(), organizationUuid: fx.orgUuid, warehouseUuid: fx.warehouseUuid,
			authorUuid: fx.userUuid, posted: true,
		},
	});
	...
	try {
		// НЕпроведённый резерв регистр НЕ движет (черновик не режет доступный остаток).
		await prisma.reservation.update({ where: { uuid: reservation.uuid }, data: { posted: false } });
		await reconcileReservationRegister(reservation.uuid);
		assert.equal(
			await reservedQuantity(product.uuid, fx.warehouseUuid, null), 0,
			"непроведённый резерв не резервирует",
		);
		await prisma.reservation.update({ where: { uuid: reservation.uuid }, data: { posted: true } });
