...
//
// Каждая задача opt-in по env (intervalMs<=0 → не регистрируется).
//
// НЕСКОЛЬКО ПРОЦЕССОВ — БЛОКИРОВКА В БАЗЕ (24.09). Раньше здесь стояло «масштаб — 1 инстанс»,
// и это было верно ровно до того дня, когда прод впервые поднимется кластером: четыре воркера
// pm2 — это четыре планировщика, то есть четыре одновременных `pg_dump` одной базы и четыре
// чистки журнала. Каждый сам по себе безобиден, вместе — нагрузка на диск и БД в самый
// неподходящий момент, потому что стартуют они синхронно.
//
// Поэтому задача может выполняться под блокировкой — её передаёт вызывающий (`withLock`).
// Реализация живёт в `services/clusterLock.js` и опирается на advisory-lock Postgres: лок
// держится в СЕССИИ и снимается сам при обрыве соединения, так что упавший воркер не оставит
// задачу заблокированной навсегда.
//
// САМ ПЛАНИРОВЩИК ПРО БАЗУ НЕ ЗНАЕТ. Это не чистоплюйство: затащив сюда Prisma, я тут же
// сломал его headless-тест — а планировщик как раз из тех мест, где проверка нужнее всего.
// Блокировки нет — работаем как раньше, одним процессом.
// ─────────────────────────────────────────────────────────────────────────────
 ...
 */
export function startScheduler({ log = defaultLog, withLock = null } = {}) {
	...
	for (const t of tasks) {
		const tick = async () => {
			...
			t.running = true;
			try {
				const r = withLock ? await withLock(t.name, () => t.run()) : await t.run();
				if (r) info(`${t.name}: ${r}`);
			} catch (e) {
