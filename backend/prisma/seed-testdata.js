...
const DOC_DEFS = {
	purchase_requisition: {
		...
		fk: "purchaseRequisitionUuid",
		vat: true,
	},
	purchase_order: {
		model: "purchaseOrder",
		item: "purchaseOrderItem",
		fk: "purchaseOrderUuid",
		vat: true,
	},
	purchase: {
		model: "purchase",
	...
	incoming_invoice: {
		...
		fk: "incomingInvoiceUuid",
		vat: true,
	},
	commercial_offer: {
		model: "commercialOffer",
		item: "commercialOfferItem",
		fk: "commercialOfferUuid",
		vat: true,
	},
	sales_order: {
		model: "salesOrder",
	...
	outgoing_invoice: {
		...
		item: "outgoingInvoiceItem",
		fk: "outgoingInvoiceUuid",
		vat: true,
	},
	payment_invoice: {
		model: "paymentInvoice",
		item: "paymentInvoiceItem",
		fk: "paymentInvoiceUuid",
		vat: true,
	},
	sale_return: {
		model: "saleReturn",
	...
	purchase_return: {
		...
		fk: "purchaseReturnUuid",
		vat: true,
	},
	inventory_transfer: {
		model: "inventoryTransfer",
		item: "inventoryTransferItem",
		fk: "inventoryTransferUuid",
		vat: false,
		transfer: true,
	},
};
...
async function createDoc(type, rawHeader, lines) {
	...
	if (items.length) {
		await prisma[def.item].createMany({
			data: items.map((it) => ({ ...it, [def.fk]: parent.uuid })),
		});
	}
...
function applyBasis(type, header) {
	...
	const { basisDocumentType, basisDocumentUuid, basisDocumentLabel, ...rest } =
		header;
	return rest;
}
...
async function cleanup() {
	...
	// 1b. Рвём «мягкие» ссылки-основания (basisDocumentUuid) у всех детей ДО удаления
	// документов. Основание — полиморфная ссылка без FK, поэтому удаление родителя
	// раньше ребёнка иначе оставило бы «висячую» ссылку. Зануляем — и порядок удаления
	// больше не важен, и не остаётся осиротевших basisDocumentUuid.
	const BASIS_CHILD_MODELS = [
		"sale",
		"purchase",
		"outgoingInvoice",
		"saleReturn",
	...
	for (const model of BASIS_CHILD_MODELS) {
		try {
			await prisma[model].updateMany({
				...byOrg,
				data: {
					basisDocumentType: null,
					basisDocumentUuid: null,
					basisDocumentLabel: null,
				},
			});
		} catch (e) {
			console.warn(
				`cleanup: не удалось занулить основание у ${model}: ${e.message}`,
			);
		}
	}
...
async function ensureGlobals() {
	...
	if (!admin)
		throw new Error("Не найден пользователь admin — запустите prisma/seed.js");
	ADMIN = admin.uuid;
	...
	if (!kzt)
		kzt = await prisma.currency.create({
			data: { code: "KZT", name: "Тенге", symbol: "₸" },
		});
...
async function createReferenceData(globals) {
	...
	for (let i = 0; i < CONFIG.organizations; i++) {
		...
		const warehouses = {
			main: await prisma.warehouse.create({
				data: { organizationUuid: org.uuid, name: "Основной склад" },
			}),
			retail: await prisma.warehouse.create({
				data: { organizationUuid: org.uuid, name: "Розничный склад" },
			}),
			materials: await prisma.warehouse.create({
				data: { organizationUuid: org.uuid, name: "Склад материалов" },
			}),
		};
		const cashboxes = [
			await prisma.cashbox.create({
				data: { organizationUuid: org.uuid, name: "Основная касса" },
			}),
			await prisma.cashbox.create({
				data: { organizationUuid: org.uuid, name: "Операционная касса" },
			}),
		];
		const bankAccounts = [];
		for (let b = 0; b < 2; b++) {
				...
				await prisma.bankAccount.create({
					data: {
						...
						iban:
							`KZ${pad(i, 2)}${pad(b, 2)}` +
							"00000000" +
							pad(randint(100000, 999999), 6),
						bik: "HSBKKZKX",
						bankName: "АО «Народный банк»",
						name: `Расчётный счёт №${b + 1}`,
						currencyUuid: globals.kztUuid,
						isPrimary: b === 0,
					},
				}),
			);
		...
		const positions = [];
		for (const pn of positionNames)
			positions.push(
				await prisma.position.create({
					data: { organizationUuid: org.uuid, name: pn },
				}),
			);
		...
		for (let e = 0; e < CONFIG.employeesPerOrg; e++) {
			...
			const emp = await prisma.employee.create({
				data: {
					...
					firstName: fn,
					fullName: `${ln} ${fn}`,
					iin: EMP_IIN_PREFIX + pad(i * 100 + e, 8),
				},
			});
		...
		const brand = await prisma.brand.create({
			data: { organizationUuid: org.uuid, name: `Бренд ${orgNames[i]}` },
		});
		...
		orgs.push({
			...
			employees,
			brand,
			counterparties: { suppliers: [], customers: [], both: [] },
			contracts: {},
			products: { goods: [], materials: [], services: [], works: [] },
		});
	}
	...
	for (let c = 0; c < CONFIG.counterparties; c++) {
		...
		const cp = await prisma.counterparty.create({
			data: {
				organizationUuid: org.rec.uuid,
				bin: CP_BIN_PREFIX + pad(c + 1, 8),
				name: `${role} «${pick(cpNames)}-${c + 1}»`,
				legalName: `ТОО «${pick(cpNames)} ${c + 1}»`,
			},
		});
		...
		for (let k = 0; k < nContracts; k++) {
			const ct = await prisma.contract.create({
				data: {
					...
					name: `Договор №${c + 1}/${k + 1}`,
					contractNumber: `${c + 1}-${k + 1}`,
	...
	const mk = async (org, kind, idx, isService, uomCode) => {
		...
		const p = await prisma.product.create({
			data: {
				organizationUuid: org.rec.uuid,
				brandUuid: org.brand.uuid,
				unitOfMeasureUuid: globals.uoms[uomCode],
				name: `${labels[kind]} ${idx} (${org.name})`,
				sku: `${kind.toUpperCase().slice(0, 3)}-${idx}`,
				isService,
			},
		});
		org.products[kind].push({ ...p, uom: globals.uoms[uomCode] });
	};
	for (let g = 0; g < CONFIG.goods; g++)
		await mk(
			orgs[g % orgs.length],
			"goods",
			g + 1,
	...
	for (let m = 0; m < CONFIG.materials; m++)
		await mk(
			orgs[m % orgs.length],
			"materials",
			m + 1,
	...
	for (let s = 0; s < CONFIG.services; s++)
		await mk(orgs[s % orgs.length], "services", s + 1, true, "356");
	for (let w = 0; w < CONFIG.works; w++)
		await mk(orgs[w % orgs.length], "works", w + 1, true, "356");

	stats.refs = {
		organizations: orgs.length,
		counterparties: await prisma.counterparty.count({
			where: { bin: { startsWith: CP_BIN_PREFIX } },
		}),
		contracts: await prisma.contract.count({
			where: { organizationUuid: { in: orgs.map((o) => o.rec.uuid) } },
		}),
...
async function createCashReceipt(rawHeader) {
	...
	const rec = await prisma.cashOrder.create({
		data: { ...header, direction: "receipt", authorUuid: ADMIN, posted: true },
	});
...
async function createCashExpense(rawHeader) {
	...
	const rec = await prisma.cashOrder.create({
		data: { ...header, direction: "expense", authorUuid: ADMIN, posted: true },
	});
...
async function createBankStatement(rawHeader) {
	...
	const rec = await prisma.bankStatement.create({
		data: { ...header, authorUuid: ADMIN, posted: true },
	});
...
const addStock = (o, w, p, q) =>
	stock.set(stockKey(o, w, p), (stock.get(stockKey(o, w, p)) || 0) + q);
...
async function purchaseChain(org, idx) {
	...
	const req = await createDoc(
		"purchase_requisition",
		{ ...baseHeader, date: D(2026, 1, day), comment: "Заявка на закупку" },
		lines,
	);
	// 2. Заказ поставщику.
	const order = await createDoc(
		"purchase_order",
		{
			...baseHeader,
			date: D(2026, 1, day + 1),
			comment: "Заказ поставщику",
			...basisOf("purchase_requisition", req),
		},
		lines,
	);
	...
	const purchase = await createDoc(
		"purchase",
		{
			...
			...basisOf("purchase_order", order),
		},
		lines,
	);
	for (const l of lines) addStock(org.rec.uuid, wh.uuid, l.product.uuid, l.qty);
	// 4. Счёт-фактура полученная.
	const inv = await createDoc(
		"incoming_invoice",
		{
			...baseHeader,
			date: D(2026, 1, day + 3),
			comment: "Счёт-фактура полученная",
			...basisOf("purchase", purchase),
		},
		lines,
	);
	...
	if (idx % 2 === 0) {
		await createBankStatement({
			...baseHeader,
			bankAccountUuid: pick(org.bankAccounts).uuid,
			direction: "bankStatementOut",
			amount: payAmount,
			date: D(2026, 1, day + 5),
			comment: "Оплата поставщику (банк)",
			...basisOf("incoming_invoice", inv),
		});
	} else {
		await createCashExpense({
			...baseHeader,
			cashboxUuid: pick(org.cashboxes).uuid,
			amount: payAmount,
			date: D(2026, 1, day + 5),
			comment: "Оплата поставщику (касса)",
			...basisOf("incoming_invoice", inv),
		});
	}
...
async function saleChain(org, idx) {
	...
	const available = org.products.goods.filter(
		(p) => getStock(org.rec.uuid, wh.uuid, p.uuid) >= 10,
	);
	...
	const baseHeader = {
		organizationUuid: org.rec.uuid,
		counterpartyUuid: customer.rec.uuid,
		contractUuid: contract.uuid,
	};
	const lines = products.map((p) => {
	...
	const serviceLines = pickN(
		[...org.products.services, ...org.products.works],
		randint(0, 2),
	).map((p) => ({
		product: p,
		uom: p.uom,
	...
	const order = await createDoc(
		"sales_order",
		{
			...
			...basisOf("commercial_offer", offer),
		},
		lines,
	);
	// 3. Резервирование товара.
	const reserve = await createDoc(
		"reservation",
		{
			...baseHeader,
			warehouseUuid: wh.uuid,
	...
	const sale = await createDoc(
		"sale",
		{
			...
			date: D(2026, 2, day + 3),
			posted: true,
			comment: "Реализация товаров и услуг",
			...basisOf("sales_order", order),
		},
		lines,
	);
	for (const l of lines)
		addStock(org.rec.uuid, wh.uuid, l.product.uuid, -l.qty);
	// 5. Электронный счёт-фактура (с услугами).
	const esf = await createDoc(
		"outgoing_invoice",
		{
			...baseHeader,
			date: D(2026, 2, day + 4),
			comment: "ЭСФ (счёт-фактура выданная)",
			...basisOf("sale", sale),
		},
		[...lines, ...serviceLines],
	);
	// 6. Счёт на оплату.
	const bill = await createDoc(
		"payment_invoice",
		{
			...baseHeader,
			date: D(2026, 2, day + 4),
			comment: "Счёт на оплату",
			...basisOf("sale", sale),
		},
		lines,
	);
	...
	if (idx % 2 === 0) {
		await createCashReceipt({
			...
			amount: payAmount,
			date: D(2026, 2, day + 6),
			comment: "Поступление оплаты (ПКО)",
			...basisOf("payment_invoice", bill),
		});
	} else {
		await createBankStatement({
			...baseHeader,
			bankAccountUuid: pick(org.bankAccounts).uuid,
			direction: "bankStatementIn",
			amount: payAmount,
			date: D(2026, 2, day + 6),
			comment: "Поступление оплаты (банк)",
			...basisOf("payment_invoice", bill),
		});
	}
...
async function transferChain(org) {
	const from = org.warehouses.main,
		to = org.warehouses.retail;
	const available = org.products.goods.filter(
		(p) => getStock(org.rec.uuid, from.uuid, p.uuid) >= 10,
	);
	...
	const lines = products.map((p) => {
		const qty = Math.max(
			...
			Math.min(
				randint(3, 15),
				Math.floor(getStock(org.rec.uuid, from.uuid, p.uuid) * 0.3),
			),
		);
		return { product: p, uom: p.uom, qty, price: randint(300, 5000) };
	});
	const t = await createDoc(
		"inventory_transfer",
		{
			organizationUuid: org.rec.uuid,
			fromWarehouseUuid: from.uuid,
			toWarehouseUuid: to.uuid,
			date: D(2026, 2, randint(24, 27)),
			posted: true,
			comment: "Перемещение на розничный склад",
		},
		lines,
	);
	for (const l of lines) {
...
async function saleReturnChain(org, sale) {
	...
	const header = {
		organizationUuid: org.rec.uuid,
		counterpartyUuid: sale.customer.rec.uuid,
		contractUuid: pick(sale.customer.contracts).uuid,
		warehouseUuid: sale.wh.uuid,
		date: D(2026, 3, randint(1, 12)),
		posted: true,
		comment: "Возврат от покупателя",
		...basisOf("sale", sale.sale),
	};
	await createDoc("sale_return", header, [
		{ product: line.product, uom: line.uom, qty, price: line.price },
	]);
...
async function purchaseReturnChain(org, purchase) {
	...
	const items = await prisma.purchaseItem.findMany({
		where: { purchaseUuid: purchase.purchase.uuid },
	});
	const it = pick(items);
	if (!it) return;
	...
	const qty = Math.max(
		1,
		Math.min(Math.floor(Number(it.quantity) * 0.15), Math.floor(maxQty)),
	);
	const header = {
		organizationUuid: org.rec.uuid,
		counterpartyUuid: purchase.supplier.rec.uuid,
		contractUuid: pick(purchase.supplier.contracts).uuid,
		warehouseUuid: wh.uuid,
		date: D(2026, 3, randint(1, 12)),
		posted: true,
	...
	await createDoc("purchase_return", header, [
		{
			product: { uuid: it.productUuid },
			uom: it.unitOfMeasureUuid,
			qty,
...
async function payrollChain(org) {
		...
		for (const emp of org.employees) {
			...
			const calc = await prisma.payrollCalculation.create({
				data: {
					...
					netSalary: net,
					totalExpense: base,
					comment: `Начисление зарплаты за ${period}`,
				},
			});
			recordDoc("payroll_calculation", calc, {
				organizationUuid: org.rec.uuid,
			});
			// Выплата: большинство — банк, часть — касса. Иногда частичная.
			const method =
				org.employees.indexOf(emp) % 3 === 0 ? "cash" : "bank_transfer";
	...
	stats.chains.payroll =
		(stats.chains.payroll || 0) +
		org.employees.length * CONFIG.payrollPeriods.length;
}
...
async function createDocuments(orgs) {
	for (const org of orgs) {
		...
		const purchases = [];
		for (let i = 0; i < CONFIG.purchaseChainsPerOrg; i++)
			purchases.push(await purchaseChain(org, i));
		// Перемещения.
		for (let i = 0; i < CONFIG.transfersPerOrg; i++) await transferChain(org);
		// Продажи (расходуют остатки).
		const sales = [];
		for (let i = 0; i < CONFIG.saleChainsPerOrg; i++) {
			const s = await saleChain(org, i);
			if (s) sales.push(s);
		}
		// Возвраты.
		for (let i = 0; i < CONFIG.saleReturnsPerOrg && i < sales.length; i++)
			await saleReturnChain(org, sales[i]);
		const goodsPurchases = purchases
			.filter(Boolean)
			.filter((p) => p.wh.uuid === org.warehouses.main.uuid);
		for (
			let i = 0;
			i < CONFIG.purchaseReturnsPerOrg && i < goodsPurchases.length;
			i++
		)
			await purchaseReturnChain(org, goodsPurchases[i]);
		// Зарплата.
...
async function postAll() {
	...
	for (const d of posted) {
		if (REGISTER_DOC_TYPES.includes(d.type)) {
			await reconcileDocumentRegister(d.type, d.uuid);
			regCount++;
		}
		if (POSTING_DOC_TYPES.includes(d.type)) {
			await reconcileDocumentEntries(d.type, d.uuid);
			entCount++;
		}
	}
...
async function integrityCheck(orgUuids) {
	const errors = [];
	const exists = async (model, uuid) =>
		uuid ? !!(await prisma[model].findUnique({ where: { uuid } })) : true;
	const modelByType = Object.fromEntries(
		Object.entries(DOC_DEFS).map(([t, d]) => [t, d.model]),
	);
	...
	for (const d of registry) {
		if (d.basisType || d.basisUuid) {
			if (!(d.basisType && d.basisUuid)) {
				errors.push(`Документ ${d.type}#${d.id}: основание заполнено частично`);
				continue;
			}
			...
			if (!base) {
				...
				if (!ok)
					errors.push(
						`Документ ${d.type}#${d.id}: основание ${d.basisType} не найдено`,
					);
			} else {
				if (base.org !== d.org)
					errors.push(
						`Документ ${d.type}#${d.id}: организация не совпадает с основанием`,
					);
				if (d.cp && base.cp && d.cp !== base.cp)
					errors.push(
						`Документ ${d.type}#${d.id}: контрагент не совпадает с основанием`,
					);
			}
		}
	}
	...
	for (const d of registry) {
		...
		while (cur && cur.basisUuid) {
			if (seen.has(cur.uuid)) {
				errors.push(`Циклическая зависимость оснований у ${d.type}#${d.id}`);
				break;
			}
			seen.add(cur.uuid);
	...
	for (const d of registry) {
		...
		const cnt = await prisma.accountingEntry.count({
			where: { documentType: d.type, documentUuid: d.uuid },
		});
	...
	for (const d of registry) {
		...
		const cnt = await prisma.productRegister.count({
			where: { documentType: d.type, documentUuid: d.uuid },
		});
		if (cnt === 0)
			errors.push(`Проведённый товарный ${d.type}#${d.id} без движений ТМЗ`);
	}
	...
	for (const [k, q] of bal)
		if (round2(q) < 0) {
			negatives++;
			if (negatives <= 5)
				errors.push(`Отрицательный остаток: ${k} = ${round2(q)}`);
		}
	...
	const analytics = await prisma.accountingEntryAnalytic.findMany({
		where: {
			subkontoType: "Counterparty",
			entry: { organizationUuid: { in: orgUuids } },
		},
		select: {
			side: true,
			objectUuid: true,
	...
	for (const a of analytics) {
		...
		const acc =
			a.side === "debit" ? a.entry.debitAccountCode : a.entry.creditAccountCode;
		const sign = a.side === "debit" ? 1 : -1;
		if (acc === "1210")
			arBal.set(a.objectUuid, (arBal.get(a.objectUuid) || 0) + sign * amt);
		if (acc === "3310")
			apBal.set(a.objectUuid, (apBal.get(a.objectUuid) || 0) + -sign * amt); // пассив: кредит +
	}
	...
	const [vatLines, noVatLines] = await Promise.all([
		prisma.purchaseItem.count({
			where: { organizationUuid: { in: orgUuids }, vatAmount: { gt: 0 } },
		}),
		prisma.purchaseItem.count({
			where: { organizationUuid: { in: orgUuids }, vatAmount: 0 },
		}),
	]);
	if (vatLines === 0)
		errors.push("Нет строк с НДС — настройки учёта не применились");
	if (noVatLines === 0)
		errors.push(
			"Нет строк без НДС — профиль «без НДС»/дата начала не применились",
		);

	const openAR = [...arBal.values()].filter((v) => round2(v) > 0.01).length;
	const openAP = [...apBal.values()].filter((v) => round2(v) > 0.01).length;
	const closedAR = [...arBal.values()].filter(
		(v) => Math.abs(round2(v)) <= 0.01,
	).length;
	const closedAP = [...apBal.values()].filter(
		(v) => Math.abs(round2(v)) <= 0.01,
	).length;
	if (openAR === 0)
		errors.push(
			"Нет открытой дебиторской задолженности (ожидались частичные оплаты)",
		);
...
function printReport(post, integrity) {
	...
	line("══════════════════════════════════════════════════════════════");
	line("\n▸ Справочники:");
	for (const [k, v] of Object.entries(stats.refs))
		line(`    ${k.padEnd(18)} ${v}`);
	line("\n▸ Документы по типам:");
	for (const [k, v] of Object.entries(stats.docs).sort())
		line(`    ${k.padEnd(22)} ${v}`);
	line(`    ${"ИТОГО документов".padEnd(22)} ${totalDocs}`);
	line("\n▸ Цепочки:");
	for (const [k, v] of Object.entries(stats.chains).sort())
		line(`    ${k.padEnd(18)} ${v}`);
	line("\n▸ Проведение:");
	...
	line("\n▸ Взаиморасчёты:");
	line(
		`    Открытая дебиторка (покупатели): ${integrity.openAR}, закрытая: ${integrity.closedAR}`,
	);
	line(
		`    Открытая кредиторка (поставщики): ${integrity.openAP}, закрытая: ${integrity.closedAP}`,
	);
	line(
	...
	if (integrity.errors.length === 0) line("    ✅ Ошибок не обнаружено");
		...
		line(`    ❌ Найдено ошибок: ${integrity.errors.length}`);
		integrity.errors.slice(0, 30).forEach((e) => line(`       • ${e}`));
	}
	line("══════════════════════════════════════════════════════════════\n");
}
...
async function main() {
	...
	for (const def of Object.values(DOC_DEFS)) {
		if (def.transfer) {
			lineTotal += await prisma[def.item].count({
				where: { inventoryTransfer: { organizationUuid: { in: orgUuids } } },
			});
			continue;
		} // у позиций перемещения нет колонки org
		lineTotal += await prisma[def.item].count({
			where: { organizationUuid: { in: orgUuids } },
		});
	}
	...
	console.log(
		`Строк документов: ${lineTotal}, проводок: ${entriesTotal}, движений ТМЗ: ${registerTotal}`,
	);
