...
**Сервису** — _закрыто: сделано 14.09 (S1–S5, `TASK_SERVICE_ECHO_WRITE_COMMANDS.md`, «Сделано — 14.09»); ниже — как было_. `parseEcho` понимал только `users` / `extensions`; `lock`, `infobases`, `config`,
...
**Проверить на сервере — окно блокировки** — _закрыто задачей А7 (агент `23:52`: при включении незаданное сбрасывается, в ответе `reset`/`note`); на `_transition` осталось проверить сброс; ниже — как было_. При снятии агент передаёт только
