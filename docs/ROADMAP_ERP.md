...
- **A0 Решения** — ⏳: ОС сервера (COM только Windows), RAS/версия платформы, служебный админ ИБ
  (единый или на базу), где хранят учётки (рекомендация — на агенте, не в БД `ai`), транспорт
  панели (рекомендация — прокси `/api1/onec/*`, а не прямой вызов `ai`).
...
- **A3 Админ-команды** — ✅ кластерная часть (2026-09-03), P1 внутрибазовая — нет.
...
- **A4 Пакетные операции** — ⏳ P1: `command_batches` + `commands.batch_id`, инструмент
  `admin_create_user` со `scope:{all|bases[]|tag}`, прогресс и отчёт по базам, повтор неуспешных.
- **A5 Админ-панель** — ✅ P0 (2026-09-03), P1 («Пользователи базы», «Задания», «Агенты») — нет.
