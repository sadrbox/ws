...
- **T7.2 (D) Виртуальный склад** — ❌ 0% (grep TaxpayerStore/Vstore = 0). Отдельный клиент
  ...
  `esf/index.js`). Референс — расширение `buhprof_esf` (`BPESF_ИнтеграцияВС`, `BPESF_ПротоколВС`, без привязки
...
### E15 — Серверный агент 1С: администрирование ~100 баз (ТЗ: `docs/TZ_ONEC_SERVER_AGENT.md`, v1.0)
Бухгалтерская компания, до 100 клиентских баз на одном сервере 1С; нужен **один агент на
сервер**, умеющий работать со всеми базами, админ-операции через чат (в т.ч. «создать
пользователя во всех базах») и админ-панель в aleppo.kz (базы, сеансы, показатели).
Сейчас в `ai/` **понятия «база» нет**: агент = одна организация = одна база
(`agents.organization_uuid NOT NULL`, `pickOnline(org)`, единственный `onec{reachable,version}`).
Два разных транспорта: кластер (RAS/`rac` — базы, сеансы, соединения; вход в базу не нужен) и
внутрибазовые операции (COM-соединение или расширение `bpapi` + служебный админ ИБ в каждой базе).
⚠ Перед подключением второй базы: `AGENT_ORG_BINDING=any` → `strict` (иначе команда уйдёт в чужую базу).
- **A0 Решения** — 🟡 (2026-09-04): ОС сервера 1С — **Windows** (значит COM для будущих `IB_*`
  доступен); платформа — **8.3.25.1257**; учётки кластера и ИБ хранятся **на агенте**
  (в переменных окружения службы; в БД `ai` — только имя сервера) — решение пользователя,
  совпадает с рекомендацией ТЗ; транспорт панели — **прямой вызов `ai`** тем же JWT, как у
  `AiAssistant` (обоснование — в A5). Открыто: подтверждение, что служба RAS поднята
  (`rac cluster list localhost:1545`), и служебный админ ИБ — единый на сервер или свой на базу
  (A0-3, блокирует A7 и блокировку сеансов).
- **A1 Модель «сервер → базы → агенты»** — ✅ (2026-09-03): миграция `004_bases.sql` (номера 002/003
  заняты), `servers`/`bases`, `agents.server_id/role/bases_synced_at`, `commands.base_key`,
  `BaseService` (`ai/src/bases/service.ts`), `pickAgentFor(org, baseKey, role)` вместо `pickOnline`,
  частичный уникальный индекс `(agent_id, base_key, request_id)` по незавершённым командам.
  Тесты маршрутизации — `ai/tests/bases.test.ts`. Исходная формулировка: миграция `002_bases.sql` (`servers`, `bases`,
  `agents.server_id/role`, `commands.base_key`), `pickOnline(org)` → `pickAgentFor(org, baseKey, role)`,
  `request_id` уникален по (агент, база). **Агентов ДВА** (§3.1 ТЗ): `business` (расширение bpapi)
  и `admin` (rac + COM) — разные службы под разными учётками ОС, разный радиус поражения.
- **A2 Протокол агента v2** — 🟡 сервер готов (2026-09-03), сторона агента — нет: `bases[]` и `role`
  в register, `bases[]` + `basesComplete` в heartbeat, ответ несёт `wantFullBases` и
  `basesFullEverySecs` (`AGENT_BASES_FULL_EVERY_SECS`, по умолчанию 300 с) — троттлингом управляет
  сервер, а не служба на сервере 1С; `baseKey` в выдаче команд; агент без `bases[]` работает как
  раньше (псевдо-база `default`). Осталось: реализация в `bpapi_agent` и ограничение параллелизма
  исполнения на его стороне.
- **A3 Админ-команды** — ✅ сервисная сторона целиком (2026-09-04): кластер + внутрибазовые
  (`IB_LIST_USERS`, `IB_CREATE_USER`, `IB_DELETE_USER`, `IB_LIST_EXTENSIONS`,
  `IB_INSTALL_EXTENSION`, `IB_DELETE_EXTENSION`) — 12 команд, гейт `cluster.admin`/`ib.admin`,
  изменяющие — `CRITICAL`. ⏳ Ждёт реализации в агенте (`docs/ONEC_AGENT_CLUSTER_CONTRACT.md`).
  Сервис: `ai/src/commands/admin.ts` (белый список, схемы payload, классы операций, гейт по
  `capabilities`), `ai/src/http/onecRouter.ts` (`/v1/onec/bases|sessions|connections`,
  снятие сеанса, блокировка входа), тесты `ai/tests/admin_commands.test.ts`.
  Агент: `bpapi_agent/src/cluster/mod.rs` — вызовы `rac` и разбор его вывода, шесть команд
  `CLUSTER_*`, коды ошибок `RAC_UNAVAILABLE/RAC_TIMEOUT/RAC_ERROR/CLUSTER_NOT_FOUND/
  INFOBASE_NOT_FOUND`; whitelist агента разделён по ролям (`Kind::capabilities_for`).
  Набор закрыт: произвольную команду `rac` выполнить нельзя, аргументы собирает код.
  Осталось по A3: внутрибазовые `IB_*` (нужен A7 — служебный админ ИБ в каждой базе).
- **A4 Пакетные операции** — ✅ (2026-09-04): миграция `005_batches_and_ib.sql`
  (`command_batches` + `commands.batch_id` + кэши `base_users`/`base_extensions`),
  `POST /v1/onec/batch` (202 + batchId сразу: сто подключений в HTTP не укладываются),
  прогресс `GET /batches/:id` считается по `commands.batch_id` (без дублирующих счётчиков),
  пропущенные базы возвращаются с причиной. UI — вкладка «Задания» с опросом и кнопкой
  «Повторить неуспешные» (`POST /batches/:id/retry`: payload берётся из самих команд —
  в задании пароль и .cfe намеренно не хранятся). ЧТЕНИЕ идёт мимо заданий, обычными
  запросами по выбранным базам (число одновременных — `ONEC_CHECK_PARALLEL`, один источник
  на сервис и панель).
- **A5 Админ-панель** — ✅ вкладки «Базы», «Сеансы», «Расширения», «Пользователи баз»,
  «Задания», «Агенты» (2026-09-05) — панель закрыта полностью. Доступ — по праву `OneCAdmin` (сервис читает
  `access_permissions` ERP) и **вне зависимости от активной организации**: сервер 1С один на
  установку. Групповые операции — по ОТМЕЧЕННЫМ базам (Table получил `onSelectionChange`).
  Пейн `frontend/src/models/OneCAdmin` (вкладки «Базы» и «Сеансы», снятие сеанса и блокировка
  входа через модальное подтверждение), сервис `frontend/src/services/onec/api.ts`, общий
  адрес AI Service вынесен в `frontend/src/services/ai/endpoint.ts` (был продублирован в
  `models/AiAssistant`), пункт NavList под правом `OneCAdmin` (суперадмин видит всегда),
  ключи RU+KK.
  **Отступление от A0-5:** панель ходит в AI Service НАПРЯМУЮ тем же JWT, как это уже делает
  `AiAssistant`, а не через прокси `/api1/onec/*` в бэкенде. Причина: прокси означал бы второй
  контур аутентификации и второй контракт при том, что сервис и так проверяет тот же JWT и
  сам пишет аудит. Если прокси всё-таки нужен (единая точка логирования на бэкенде) — это
  отдельная небольшая задача, фронт менять не придётся, только базовый адрес.
- **A6 Безопасность/лицензии** — ✅ кроме COM-пула (2026-09-03): гейт по `capabilities` до
  постановки команды (`agentCanRun`), аудит каждой админ-команды (`onec.admin`), ограничение
  параллелизма на агенте (`agent.max_parallel`, по умолчанию 4; следующая порция команд не
  берётся, пока не отработала предыдущая), rate limit обращений к кластеру НА ОРГАНИЗАЦИЮ
  (`RATE_LIMIT_ONEC_CLUSTER_PER_MIN`, 60/мин; локальное чтение реестра баз не считается).
  Пул COM-соединений появится вместе с внутрибазовыми `IB_*` — сейчас COM никто не открывает.
- **Инструмент диагностики** — ✅ (2026-09-05): `npm run onec-probe` (`ai/tools/onec_probe.ts`)
  — прогон читающих команд по базам мимо HTTP и прокси, таблица «база → итог → время»,
  группировка ошибок по коду, `--parallel` для поиска потолка одновременности.
  ⚠ Измерено: вход в базу занимает от 20 с до **881 с**, поэтому ожидание ответа в HTTP
  ограничено `ONEC_COMMAND_TIMEOUT_SECS`=45 (заведомо меньше лимита Cloudflare ~100 с —
  иначе прокси обрывает запрос СВОИМ ответом без CORS-заголовков).
- **A7 Раскатка на 100 баз** — ⏳ P1: инвентаризация через `rac`, пакетное создание служебного
  админа ИБ, пакетная установка расширения `bpapi` + версия расширения в heartbeat (видно, где
  отстало).

### E16 — Развёртывание и модульная поставка (план: `docs/PLAN_DEPLOY_AND_MODULES_2026-09-24.md`)
Задача владельца (24.09): помощник установки на чужие веб-серверы и модули, которые не скрываются,
а ставятся и снимаются. Решения: данные при снятии **сохраняются** (отчётность прошлых периодов
важнее чистоты базы), управление — **по организациям**, помощник — **и скрипт, и Docker Compose**.

Уровня два, и это не усложнение: **установка** решает состав поставки (таблицы, чанки, роутеры),
**организация** — кто чем пользуется среди установленного. Схема БД и сборка фронта одни на установку,
поэтому физическое отсутствие модуля возможно только на её уровне; для организации отсутствие — полное
в работе (меню, пейны, все маршруты, отчёты, «на основании», поиск, инструменты помощника).

**Подготовка до установщика** — `docs/DESIGN_PREINSTALL_AUDIT_2026-09-24.md` (проект, 24.09): разбор
сценариев пользователей и самостоятельности установки. Семь работ обязательны ДО первой чужой установки:
запрет прав по умолчанию (сейчас незнакомый путь проходит), убрать всевластие `admin` вне production,
production по умолчанию в `ecosystem`, адреса из конфигурации вместо `api.aleppo.kz` и двух наших IP,
идентификатор установки со сценарием клона, гард модуля на всех методах (сейчас только POST 11 путей),
отчёты под право и модуль (сейчас открыты всем в организации).

**Три режима установки** — `docs/PLAN_INSTALL_MODES_2026-09-24.md` (план задач, 24.09): консалтинг
(БухПроф ведёт учёт клиентов), изолированные организации (один сервер — много чужих друг другу арендаторов),
группа (одна или несколько организаций для одной группы пользователей). Режим — ОДНА настройка установки, а
не три продукта: различий ровно пять (кто заводит организацию, открыта ли регистрация с экрана входа, кто
кого видит, что в справочнике организаций, как переключаются организации). Модель доступа для аутсорсинга —
`docs/DESIGN_TENANCY_AND_SERVICE_ORG_2026-09-24.md`. **Первая задача — О1:** `/auth/register` не создаёт
`AccessRight`, и создатель организации остаётся без организации и прав на первом же запросе.

- **A1–A2 Манифест и `doctor`** — ⏳ основание для всего остального.
- **A3–A4 CLI-установщик и веб-мастер первого запуска** — ⏳ после них систему может поставить не автор.
- **A5–A8 Compose, обновление, документ, проверка установщика в CI** — ⏳.
- **Б2 Границы модулей** — ⏳ самая долгая работа: 7 ключей модулей на 101 модель и 110 роутеров,
  остальное не отнесено никуда. Без разметки части Б строить не на чем.
- **Б1, Б3 Описание модуля и зависимости** — ⏳ один источник вместо двух ручных списков (T11.1).
- **Б4 Настоящее отсутствие у организации** — ⏳ углубление T11.1: сейчас это меню плюс 403 на POST.
- **Б5 Состав схемы и сборки** — ⏳ самое рискованное, делать последним: 91 миграция линейна, модульный
  состав возможен только для НОВЫХ установок.
- **Б6–Б8 Правило о данных, пейн «Модули», тесты согласованности** — ⏳.

### E17 — Стандарт качества БухПроф: поручения, проверки учёта, нарушения и бонус (план: `docs/PLAN_QUALITY_STANDARD_2026-09-25.md`)
Задача владельца (25.09): утверждён стандарт из 40 пунктов (текст — приложение А плана). Один подтверждённый
факт нарушения — бонус сотрудника за месяц не начисляется. ERP должна:
- доводить поручения клиентов до результата: SLA, напоминания, эскалация, контроль после передачи;
- сама находить проблемы учёта в базах 1С клиентов;
- вести реестр нарушений и считать итог бонуса.

**Автоматика только предлагает кандидатов, нарушение подтверждает человек.** Покрытие пунктов: 21 — кандидата
создаёт правило, 13 — система даёт сигнал и данные, 6 — только ручная фиксация.
**Статус: ✅ реализовано 25.09 (backend, сервис ai, панель; миграция применена); 1С сдала волну 1 проверок
учёта — ⏳ живой прогон, следующие волны 1С и решения владельца.** Включается назначением организации-фирмы в «Качество → Настройки качества» (до этого
правила молчат, задним числом не действуют). Что осталось и почему — раздел «Реестр „Проверить потом“» плана.
25.09 (вечер) по большей части реестра приняты «лучшие решения по умолчанию»:
- сроки — в рабочем времени фирмы по производственному календарю РК;
- обращение определяется по автору;
- свой статус отмены;
- проверяются только обслуживаемые клиенты, «база не проверена» видна на панели;
- выгрузка итогов месяца в Excel;
- проверка ответа клиенту моделью;
- назначение фирмы в один шаг.

- **СК0 Подготовка** — ✅ (экран ServiceLink — нет, ответственный задаётся в группе):
  - закрытость задач по `isFinal`, а не жёстким статусам в `reports.js`;
  - мягкое удаление задач;
  - поля `startedAt`/`completedAt`;
  - группы сотрудников и роль главбуха;
  - экран `ServiceLink`/`ServiceAssignment` (сейчас есть только API);
  - Telegram-уведомления.
- **СК1 Обращения и доведение до результата** — ✅: вид задачи и SLA, обязательный результат, «Напомнить»,
  журнал передачи и наблюдатели, ошибки с задачей-проверкой, эскалация и «Нужна помощь», исполнитель
  `ScheduledTask.cronExpr` для регламентных задач.
- **СК2 Проверки учёта в базах 1С** — ✅ сервис и ERP, ✅ 1С волна 1 (25.09), ⏳ 1С волны 2–4.
  - Сторона 1С: `docs/TASK_EXTENSION_ACCOUNTING_CHECKS_2026-09-25.md`, передана в сессию расширения 25.09.
    Это три READ-команды (`LIST_ACCOUNTING_CHECKS`, `RUN_ACCOUNTING_CHECK`, `GET_ACCOUNTING_SNAPSHOT`), около
    двадцати проверок волнами и устойчивый `fingerprint` находки.
  - Сервис: ночной прогон и хранение находок в `ai`.
  - ERP: находка → задача, закрывает её прогон; исключения с причиной; сверка налогов с лицевым счётом КН.
  - Ответ 1С по волне 1 — `docs/TASK_SERVICE_ACCOUNTING_CHECKS_2026-09-25.md`: расширение 1.7.3, агент
    `2026-09-25 21:58`, одиннадцать проверок, проверены на `Dev_01`. Сервис по нему: коды «не умеет» от агентов
    11:56–21:58, «Нет прав в 1С» на панели главбуха, пп. 27/28 только по ошибкам. Живого прогона через сервис
    ещё не было. Дальше у 1С: волна 2 (ПУ3) и снимки (ПУ5), затем ЭСФ (ПУ4).
- **СК3 Чек-листы самопроверки** — ✅: «ок» нельзя поставить при открытых находках. Если находка была до
  отметки, это кандидат по пп. 27–28.
- **СК4 Панели главбуха и руководителя** — ✅.
- **СК5 Реестр нарушений и бонус (ядро)** — ✅: справочник пунктов, нарушение с доказательствами, подтверждение
  по уровням и оспаривание, меры руководителя, закрытие месяца, флаг систематичности.
- **СК6 Трудовая дисциплина** — ✅: график, отметка прихода (кнопка или первый вход), заявки на опоздание и
  отсутствие; праздники и переносы — по производственному календарю РК.
- **СК7 Качество консультаций** — ✅: эвристики формы и «Проверить моделью» (сервис ai, по кнопке), оценка
  клиента; правильность нормы модель не подтверждает — нужен источник текстов НПА.

Порядок:
1. СК0 + СК5 с ручным реестром — честный учёт бонуса с первого месяца.
2. СК1.
3. СК2 и СК4.
