...
- **A1 Модель «сервер → базы → агенты»** — ⏳ P0: миграция `002_bases.sql` (`servers`, `bases`,
...
- **A2 Протокол агента v2** — ⏳ P0: `bases[]` в register/heartbeat (+троттлинг: 100 баз нельзя
  опрашивать каждые 30 с), команды по разным базам, ограничение параллелизма, совместимость
  со старым агентом (псевдо-база `default`).
- **A3 Админ-команды** — ⏳ P0 (кластер: `CLUSTER_LIST_INFOBASES/SESSIONS/CONNECTIONS`,
  `TERMINATE_SESSION`, `SET_SESSIONS_LOCK`) / P1 (внутрибазовые: `IB_LIST_USERS`,
  `IB_CREATE_USER/UPDATE_USER/DISABLE_USER`). Опасные — класс `CRITICAL` (карточка подтверждения
  в `chat/workflow.ts` уже есть).
...
- **A5 Админ-панель** — ⏳ P0 (вкладки «Базы», «Сеансы») / P1 («Пользователи базы», «Задания»,
  «Агенты»): пейн `models/OneCAdmin`, право `OneCAdmin`, пункт NavList.
- **A6 Безопасность/лицензии** — ⏳ P0 сквозная: гейт по `capabilities`, пул COM-соединений
  (COM занимает лицензию 1С), аудит админ-команд, rate limit на опрос кластера.
