...
const schema = z.object({
	...
	/*
	 * Сколько ждать 1С, когда карточка организации читает долги и остатки (ПН9). Дольше обычных команд
	...
	/*
	 * Сколько файлов принимать в ОДНОМ сообщении чата 1С. Тройка, стоявшая в схеме, была взята под сценарий
	 * «прислал выписку — разобрали»; бухгалтер шлёт пачку за месяц, и предел превращался в пять сообщений
	 * подряд. Вложения едут идентификаторами (загружаются отдельно), поэтому двадцать файлов не делают тело
	 * хода тяжелее.
	 */
	CHAT_ATTACHMENTS_MAX: z.coerce.number().int().min(1).max(100).default(20),
	/*
	 * Предел тела JSON у всех маршрутов, кроме веб-чата ERP (у того свой: там вложения едут base64).
	 * Ход канала 1С сюда укладывается: вложения идут идентификатором, а результаты вызовов формы приходят
	 * БЕЗ файла — по контракту канала (CONTRACT_1C_CHAT, «файлы в сервис не отправляются») форма кладёт
	 * вместо содержимого `{contentOmitted: true, bytes}`.
	 *
	 * ПЯТЬ, А НЕ ДВА (аудит 22.09): в одном ходе бывает до 50 результатов вызовов, и у каждого `message`
	 * может быть до 20 000 символов, а `data` схемой не ограничена вовсе — двух мегабайт такому ходу мало.
	 * Пять совпадают с пределом, на котором режет тело само расширение: так обе стороны отказывают на одном
	 * и том же размере, а не поочерёдно.
	 */
	JSON_BODY_MAX_MB: z.coerce.number().int().min(1).max(64).default(5),
	...
	/*
	 * ЗАГРУЗОК ВЛОЖЕНИЙ В МИНУТУ на пару «база + пользователь». Шесть, стоявшие здесь, спорили с пределом
	 * числа файлов в сообщении (CHAT_ATTACHMENTS_MAX = 20): человек прикладывал пачку выписок за месяц и
	 * после седьмого файла получал «подождите минуту» — предел, о котором он не просил и которого не видит.
	 * Шестьдесят — три полные пачки в минуту: злоупотребление по-прежнему отсекается, обычная работа нет.
	 */
	RATE_LIMIT_ATTACHMENTS_PER_MIN: z.coerce.number().int().min(0).max(300).default(60),
