...
export type LLMRequest = {
	...
	cacheable?: boolean;
	/**
	 * Изменчивая часть системного промпта (контекст организации: её задачи и заметки). Отдельно от
	 * `system`, потому что кэшируется именно стабильный префикс: подмешай это в него — и кэш
	 * промахивался бы на каждом ходе.
	 */
	systemExtra?: string;
	maxTokens?: number;
};
